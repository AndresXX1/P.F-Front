...
const Shopping = () => {
    ...
    const [quantities, setQuantities] = useState(
      visibleItems.reduce((acc, item) => ({ ...acc, [item.id]: item.quantity }), {})
    );
  
    const total = visibleItems.reduce((accumulator, item) => {
      return accumulator +(item.price * (  quantities[item.id] ? quantities[item.id] : item.quantity ));
    }, 0);
  
    const mercadoPago = async () => {
      try {
        const response = await axios.post(
          `http://localhost:3000/payment/createOrder/${auth.token.id}`,
          {
          ...
          <div className={style.tableRow}>
            {visibleItems.map((item) => (
              <div key={item.id} className={style.shoppingCard}>
                <img
                  src={item.Product.image.secure_url || item.Product.image[0].secure_url}
                  alt={item.Product.name}
                  className={style.cardImage}
                />
                <div className={style.cardContent}>
                  <div className={style.cardTitle}>{item.name}</div>
                  <div className={style.cardPrice}>${item.price * (quantities[item.id] ? quantities[item.id] : item.quantity )} USD</div>
                  <div>{item.quantity > 1 ? `valor unidad : ${item.price}` : null}</div>
                  ...
                  <div className={style.quantityButtons}>
                    ...
                    Amount: {quantities[item.id] && quantities ? quantities[item.id] : item.quantity}
                    <button onClick={() => handleQuantityChange(item.id, quantities[item.id] + 1)}>+</button>
                  </div>
                  <button
