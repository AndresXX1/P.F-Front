...
export default function ImgMediaCard(props) {
    ...
    const deleteElement = async (idKey) => {
      try {
        const response = await axios.delete(`http://localhost:3003/products/delete/${idKey}`);
  ...
  return (
    <Card sx={{ maxWidth: 345 }}>
      ...
      <CardActions>
        <Button size="small" onClick={() => {deleteElement(data.id)}} >Borrar</Button>
        <Button size="small" onClick={() =>{handleEdit(data)}} >Editar</Button>
      </CardActions>
    </Card>
  );
}